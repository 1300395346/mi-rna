...
import com.rna.springwebsocket.message.AuthRequest;
import com.rna.springwebsocket.message.AuthResponse;
import com.rna.springwebsocket.message.UserJoinNoticeRequest;
    ...
    public void execute(Session session, AuthRequest message) {
        ...
        if (StringUtils.isEmpty(message.getAccessToken())) {
            ...
            WebSocketUtil.send(session, AuthResponse.TYPE,
                    new AuthResponse(1,"认证 accessToken 未传入"));
            return;
        }
        ...
        WebSocketUtil.broadcast(UserJoinNoticeRequest.TYPE,
                new UserJoinNoticeRequest(message.getAccessToken())); // 考虑到代码简化，我们先直接使用 accessToken 作为 User
    }
